window.onload = function () {
    ...
    let total_forms = parseInt($('input[name=orderitems-TOTAL_FORMS]').val()) //узнаем кол-во форм

    let order_total_cost = parseInt($('.order_total_cost').text().replace(',', '.')) || 0;
    // достаем из формы общую сумму
    let order_total_quantity = parseInt($('.order_total_quantity').text()) || 0;
    // достаем из формы общее кол-во товаров
    ...
    //1 METHOD автоматический пересчет цены в зависимости от цены
    $('.order_form').on('click', 'input[type=number]', function () {
        let target = event.target;
        orderitem_num = parseInt(target.name.replace('orderitems-', '').replace('-quantity', ''))
        //получаем номер строки при нажатии input[type=number]
    ...
    //2 METHOD при удалении товаров пересчитывается общее кол-во и общая цена
